...
async function facesIn(buffer, maxDim, options) {
  ...
  try {
    ...
    const minFacePx = job.minFacePx ?? 30;
    return found
      .filter((r) => Math.min(r.detection.box.width, r.detection.box.height) >= minFacePx)
      .map((result) => ({
...
process.on("message", async (job) => {
  try {
    ...
    const options = new faceapi.SsdMobilenetv1Options({
      minConfidence: job.minConfidence ?? 0.55,
    });
    ...
    for (const url of job.urls) {
      try {
        ...
        const found = await facesIn(buffer, job.maxDim ?? 1600, options);
