...
/**
 * Images accepted in one blocking request.
 *
 * On the free tier a photo costs several seconds, so eight is already around
 * fifty seconds of request — past what most callers' own timeouts allow. Send
 * more than a handful to POST /api/jobs instead.
 */
export const MAX_SYNC_IMAGES = 8;
/** Images accepted for a background job. */
export const MAX_JOB_IMAGES = 250;

/**
 * Longest side images are scaled to before detection.
 *
 * 1600 was measured taking a 512MB instance down on a batch of eight real
 * photos — a hard restart, not a clean error, which also lost whatever was
 * queued. Detection quality is flat across this range anyway: the same faces
 * are found at 1024 as at 1600, only faster and in less memory.
 */
const MAX_DIM = Number(process.env.API_MAX_DIM ?? 1024);
...
export function createApiRouter() {
  ...
  router.get("/status", (_req, res) => {
    res.json({
      ...
      maxDim: MAX_DIM,
      chunkSize: Math.max(1, Number(process.env.API_CHUNK_SIZE ?? 4)),
